...
#define MAVLINK_MSG_ID_RUNCMD_LEN 16
#define MAVLINK_MSG_ID_RUNCMD_MIN_LEN 16
#define MAVLINK_MSG_ID_2_LEN 16
#define MAVLINK_MSG_ID_2_MIN_LEN 16

#define MAVLINK_MSG_ID_RUNCMD_CRC 153
#define MAVLINK_MSG_ID_2_CRC 153
...
#define MAVLINK_MESSAGE_INFO_RUNCMD { \
    ...
    {  { "cmd", NULL, MAVLINK_TYPE_UINT32_T, 0, 0, offsetof(mavlink_runcmd_t, cmd) }, \
         ...
         { "freq", NULL, MAVLINK_TYPE_INT32_T, 0, 12, offsetof(mavlink_runcmd_t, freq) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_RUNCMD { \
...
static inline uint16_t mavlink_msg_runcmd_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               uint32_t cmd, uint32_t dir, int32_t distance, int32_t freq)
{
...
{
    return mavlink_msg_runcmd_pack(system_id, component_id, msg, runcmd->cmd, runcmd->dir, runcmd->distance, runcmd->freq);
}
...
{
    return mavlink_msg_runcmd_pack_chan(system_id, component_id, chan, msg, runcmd->cmd, runcmd->dir, runcmd->distance, runcmd->freq);
}
...
static inline void mavlink_msg_runcmd_send(mavlink_channel_t chan, uint32_t cmd, uint32_t dir, int32_t distance, int32_t freq)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_RUNCMD_LEN];
    _mav_put_uint32_t(buf, 0, cmd);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_runcmd_send(chan, runcmd->cmd, runcmd->dir, runcmd->distance, runcmd->freq);
#else
 ...
 */
static inline void mavlink_msg_runcmd_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint32_t cmd, uint32_t dir, int32_t distance, int32_t freq)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field freq from runcmd message
 *
 * @return   freq 
 */
static inline int32_t mavlink_msg_runcmd_get_freq(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_runcmd_decode(const mavlink_message_t* msg, mavlink_runcmd_t* runcmd)
{
    ...
    runcmd->freq = mavlink_msg_runcmd_get_freq(msg);
#else
